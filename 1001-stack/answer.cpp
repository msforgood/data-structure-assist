...
bool is_matched(const std::string &expression) {
    ...
    for (char ch : expression) {                         
        if (ch == '(' || ch == '[' || ch == '{') {       // 열린 괄호라면
            st.NEED_TO_SOLVE(ch);                                 // 스택에 문자 1개를 '배열 슬롯에 직접 저장'.
