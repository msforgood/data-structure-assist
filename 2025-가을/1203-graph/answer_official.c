...
void insert_edge_matrix(GraphMatrix *g, int u, int v) { // 정점 u와 v 사이에 간선을 추가하는 함수
    ...
    g->adj_mat[u][v] = 1;                                // u에서 v로 가는 간선이 있음을 1로 표시
    g->adj_mat[v][u] = 1;                                // 무방향 그래프이므로 v에서 u로도 간선이 있음을 1로 표시
...
void init_list(GraphList *g) {                        // GraphList 포인터를 받아 그래프를 초기화하는 함수
    g->n = 0;                                         // 처음에는 정점 개수가 0개이므로 0으로 초기화
    for (int i = 0; i < MAX_VERTICES; i++) {          // 모든 정점 인덱스를 순회하는 반복문
        g->adj_list[i] = NULL;                        // 각 정점의 인접 리스트 시작 포인터를 NULL로 초기화
    }                                                 // for문 끝
}                                                     // init_list 함수 끝

// 인접 리스트 그래프에 정점 하나를 추가하는 함수
void insert_vertex_list(GraphList *g) {               // 인접 리스트 그래프에 새 정점을 추가하는 함수
    if (g->n + 1 > MAX_VERTICES) {                    // 정점 개수가 최대치를 넘는지 검사
        printf("정점 개수 초과\n");                   // 초과 시 오류 메시지 출력
        return;                                       // 함수 종료
    }                                                 // if 문 끝
    g->adj_list[g->n] = NULL;                         // 새로 추가되는 정점의 인접 리스트를 NULL로 초기화
    g->n++;                                           // 정점 개수를 1 증가
...
int main(void) {                                   // 프로그램의 시작점인 main 함수 정의
    ...
    insert_edge_list(&g_list, 1, 3);
    insert_edge_list(&g_list, 0, 2);
    insert_edge_list(&g_list, 2, 1);
